...
send_request() {
    ...
    if [ "$action" = "pre" ]; then
        endpoint="${SERVER_URL}${ALLOCATE_ENDPOINT}/${job_name}/${preference}"
    else
        endpoint="${SERVER_URL}${RELEASE_ENDPOINT}/${job_name}"
    fi
    ...
    while true; do
        ...
        if [ "$http_status" -eq 200 ]; then
            ...
            log_message "$action" "$job_name" "allocate" "" "pending" "retrying"
...
# Main script
if [ "$#" -lt 2 ] || [ "$#" -gt 3 ]; then
    echo "Error: Two or three arguments are required."
    echo "Usage: $0 <pre|post> <submit_file> [preference]"
    exit 1
fi

ACTION="$1"
SUBMIT_FILE="$2"
PREFERENCE="${3:-balanced}"  # Default to 'balanced' if not provided
...
if [[ "$PREFERENCE" != "performance" && "$PREFERENCE" != "energy" && "$PREFERENCE" != "balanced" ]]; then
    echo "Error: Invalid preference. Must be 'performance', 'energy', or 'balanced'."
    exit 1
fi
...
if [ "$ACTION" = "pre" ]; then
    resource=$(send_request "$ACTION" "$JOB_NAME" "$PREFERENCE")
